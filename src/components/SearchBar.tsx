...
function SearchBar(props: { cafeList: Mungple[], selectId: (data: Mungple) => void }) {
  ...
  const autoCompleteContext = option.map((cafe) => {
    ...
    return <div className="search-auto-item" onClick={onClickHandler}>{cafe.placeName}</div>
  })
  ...
  return (
    <div ref={ref}>
      <div className="search">
        <input id="search" value={enteredInput} onChange={inputChangeHandler} onClick={inputFoucs} />
      </div>
      {isFocus && option.length > 0 && enteredInput.length > 0 && <div className="search-auto">
        {autoCompleteContext}
      </div>
      }
    </div>);
